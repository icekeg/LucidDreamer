...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Row():
        with gr.Column():
                gaussians_path = gr.Textbox(label='ply path', value='')
                render_camerapath2 = gr.Textbox(label='video camera path', value='360_fov1.2')
                with gr.Row():
                    img_name = gr.Textbox(label='image name', value='pic01')
                    camera_angle_x = gr.Number(label='camera angle x', value=0.9934924659449375)
                with gr.Row():
                    cam_yaw = gr.Slider(label='yaw', minimum=-1.570795, maximum=1.570795,value=0,step=0.001)
                    cam_pitch = gr.Slider(label='pitch', minimum=-3.14159, maximum=3.14159,value=-3.14159,step=0.001)
                    cam_roll = gr.Slider(label='roll', minimum=-3.14159, maximum=3.14159,value=-3.14159,step=0.001)
                with gr.Row():
                    cam_posx = gr.Number(label='pos x', value=0.0)
                    cam_posy = gr.Number(label='pos y', value=0.0)
                    cam_posz = gr.Number(label='pos z', value=0.0)
        load_ply_file = gr.File(label='Gaussian splatting PLY', show_label=True)
        img_result = gr.Image(type="filepath", label="result image", height=384, width=384)
    with gr.Row():
        load_button = gr.Button(value='load gaussians data', elem_id='load-button')
        render_button2 = gr.Button(value='Render Video')
        create_image = gr.Button(value='render specific image')
                

    ips = [example_name, input_image, prompt, n_prompt, gen_camerapath, seed, diff_steps, render_camerapath, model_name]

    load_button.click(fn=ld.load, inputs=[ips[-1],gaussians_path],outputs=[load_ply_file])
    ...
    render_button2.click(fn=ld.render_video, inputs=[render_camerapath2, ips[0]], outputs=[result_gallery, result_depth])
    create_image.click(fn=ld.generate_image,inputs=[img_name,camera_angle_x,cam_roll,cam_pitch,cam_yaw,cam_posx,cam_posy,cam_posz], outputs=[img_result])
